...
use capture::presets;
use capture::{
    ScreenCapture, build_output_path, save_image_or_webp, validate_encoder, validate_format,
...
struct Args {
    ...
    #[arg(
        ...
        long,
        default_value = "jxl",
        help = "Output format: jxl, webp, png, jpg, jpeg, gif, bmp, tiff, tif (jxl needs cjxl/djxl installed)"
    )]
    format: String,
    ...
    #[arg(
        long,
        help = "Convert an existing image file, or every image in a folder, to --format (default: jxl)"
    )]
...
/// Saves the capture and reports every file written, since a tall capture is
/// split across several numbered files.
fn save_and_report(image: &image::RgbaImage, output_path: &str) -> Result<()> {
    let written = save_image_or_webp(image, output_path)?;
    if written.len() > 1 {
        info!(
            "Image is {}px tall, past the {}px page limit — split into {} parts",
            image.height(),
            capture::WEBP_MAX_DIMENSION,
            written.len()
        );
    }
    for path in &written {
...
fn main() -> Result<()> {
    ...
    if let Some(convert_path) = &args.convert {
        ...
        if std::path::Path::new(convert_path).is_dir() {
            ...
            let (converted, skipped, failed) = capture.convert_images_in_folder(
                convert_path,
                &args.format,
                args.delete_original,
                |_, _, _| {},
            )?;
            if converted + skipped + failed == 0 {
                info!("No images found in {}", convert_path);
            } else if failed > 0 {
                warn!(
                    "Done: {} converted, {} skipped, {} FAILED — see the errors above",
