...
use crate::constants::{CaptureTimings, defaults, gui as gui_const};
use eframe::egui;
...
impl Default for CaptureConfig {
    fn default() -> Self {
        Self {
            ...
            output_format: "jxl".to_string(),  // Default format
            ...
            post_capture_delay: defaults::POST_CAPTURE_DELAY,
...
impl CaptureApp {
    ...
    fn start_capture(&mut self) {
        ...
        if let Err(e) = crate::validate_encoder(&self.config.output_format) {
            *self.status.lock().unwrap() = CaptureStatus::Error(format!("{}", e));
            return;
        }
        let output_path =
    ...
    fn run_capture(
        ...
        *status.lock().unwrap() = CaptureStatus::Running("Capturing screenshots...".to_string());
        ...
        capture.log(
            log::Level::Info,
            &format!(
                "Max scrolls: {:?}, Overlap: {}px, delays: scroll delay {}ms, post capture {}ms",
                max_scrolls
        ...
        *status.lock().unwrap() = CaptureStatus::Running("Saving image...".to_string());
        ...
        crate::save_image_or_webp(&result_image, &output_path)
    }
}
...
impl CaptureApp {
    fn render_capture_tab(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ...
        ui.group(|ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("2. Screenshot to next scroll key (ms):");
                ui.add(egui::Slider::new(
            ...
            let cycle = self.config.scroll_delay + self.config.post_capture_delay;
            ui.label(
...
impl CaptureApp {
            ...
            let fix_input_row = ui.horizontal(|ui| {
                ...
                if ui.button("Browse...").clicked() {
                    if self.config.fix_folder_mode {
                        ...
                        .add_filter(
                            "Images",
                            &["jxl", "png", "jpg", "jpeg", "webp", "bmp", "tiff"],
                        )
                        .pick_file()
                    {
                        if let Some(s) = path.to_str() {
    ...
    fn load_trim_preview(&mut self, ctx: &egui::Context) {
        ...
        let img = match crate::open_image(&self.trim_input_path) {
            Ok(i) => i,
            Err(e) => {
                self.trim_status = format!("{}", e);
                return;
            }
        };
            ...
            let convert_input_row = ui.horizontal(|ui| {
                ...
                if ui.button("Browse...").clicked() {
                    if self.config.convert_folder_mode {
                        ...
                        .add_filter("Images", &["jxl", "png", "jpg", "jpeg", "webp", "bmp", "tiff"])
                        .pick_file()
                    {
                        if let Some(s) = path.to_str() {
                            self.config.convert_input_path = s.to_string();
        ...
        *status.lock().unwrap() = CaptureStatus::Running("Starting...".to_string());
        ...
        thread::spawn(move || {
            ...
            if folder_mode {
                ...
                match result {
                    Ok((converted, skipped, failed)) if converted + skipped + failed == 0 => {
                        *status.lock().unwrap() =
                            CaptureStatus::Completed("No images found in folder.".to_string());
                    }
                    // A failure must not read as a quiet success, so it is
                    // surfaced as an error even though the run finished.
                    Ok((converted, skipped, failed)) if failed > 0 => {
                        *status.lock().unwrap() = CaptureStatus::Error(format!(
                            "{} converted, {} skipped, {} FAILED — see the log",
                            converted, skipped, failed
                        ));
                    }
                    Ok((converted, skipped, _)) => {
                        *status.lock().unwrap() = CaptureStatus::Completed(format!(
